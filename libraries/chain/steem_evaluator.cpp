...
void delegate_vesting_shares(database& _db, const chain_properties& median_props, const auto& op,
        std::function<void(vesting_delegation_object& o)> add_fields_to_vdo) {
    ...
    if (delegation) {
        ...
        void delegate_vesting_shares_evaluator::do_apply(const delegate_vesting_shares_operation& op) {
            ...
            auto add_fields_to_vdo = [&](vesting_delegation_object& o) {
        ...
        void delegate_vesting_shares_with_interest_evaluator::do_apply(const delegate_vesting_shares_with_interest_operation& op) {
            ...
            auto add_fields_to_vdo = [&](vesting_delegation_object& o) {
                o.interest_rate = std::min(op.interest_rate, median_props.max_delegated_vesting_interest_rate);
            };
            delegate_vesting_shares(_db, median_props, op, add_fields_to_vdo);
        }
        ...
        void reject_vesting_shares_delegation_evaluator::do_apply(const reject_vesting_shares_delegation_operation& op) {
            ...
            if (delegation == nullptr) {
                GOLOS_THROW_MISSING_OBJECT("vesting_delegation_object", op.delegator);
            }

            _db.modify(delegator, [&](account_object& a) {
                a.delegated_vesting_shares -= delegation->vesting_shares;
            });
